...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        invoiceViewModel.allInvoices.observe(this, { i -> i?.let { adapter.submitList(it) } })
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, intentData: Intent?) {
        ...
        if (requestCode == newInvoiceActivityRequestCode && resultCode == Activity.RESULT_OK) {
            val payee = intentData?.getStringExtra(NewInvoiceActivity.EXTRA_PAYEE)!!
            val type = intentData.getIntExtra(NewInvoiceActivity.EXTRA_TYPE, 0)
            val amount = intentData.getDoubleExtra(NewInvoiceActivity.EXTRA_AMOUNT, 0.0)
            val due = intentData.getLongExtra(NewInvoiceActivity.EXTRA_DUE, 0)

            Log.d(TAG, "Received from intent PAYEE: $payee")
            Log.d(TAG, "Received from intent TYPE: $type")
            Log.d(TAG, "Received from intent AMOUNT: $amount")
            Log.d(TAG, "Received from intent DUE: $due")

            try {
                val invoice = InvoiceEntity(0, payee, type, amount, Date(due))
    ...
    fun deleteInvoice(item: InvoiceEntity) {
        invoiceViewModel.remove(item)
